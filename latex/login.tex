...
\subsubsection{新浪微博的好友匹配}

\begin{table}[H]
   \begin{center}
\begin{tabular}{|c|c|c|p{12cm}|}
\hline
GET & \multicolumn{3}{|c|}{/weibo/relation} \\
\hline\hline
 \  参数  & 类型 & 必填 &  说明  \\
\hline
    user\_id  & 字符串 & 必须 &  当前用户的id\\
\hline
    wb\_uid  & 字符串 & 必须 &  新浪微博的uid\\    
\hline
\end{tabular}
...
返回值格式：
{
to_add: [脸脸用户信息列表]   ；表示我关注微博用户也加入了脸脸的但是我还没有关注的，可以直接关注
to_invite:[微博用户信息列表]  ；表示我的微博好友还没有加入脸脸的，可以发微博邀请其加入脸脸
friend: [脸脸用户信息列表]   ；表示我关注微博用户在脸脸上我已经关注他了，仅用于显示
}
...
\subsubsection{通讯录好友匹配}

\begin{table}[H]
   \begin{center}
\begin{tabular}{|c|c|c|p{12cm}|}
\hline
GET & \multicolumn{3}{|c|}{/phone/relation} \\
\hline\hline
 \  参数  & 类型 & 必填 &  说明  \\
\hline
    user\_id  & 字符串 & 必须 &  当前用户的id\\
\hline
    phone  & 字符串 & 必须 &  绑定的手机号码\\    
 \hline
 mac  & 字符串 & 必须 &  无线网卡MAC地址的MD5\\
\hline
\end{tabular}
...
返回值格式：
{
to_add: [脸脸用户信息列表]   ；表示我的通讯录上的人也加入了脸脸的但是我还没有关注的，可以直接加好友的
to_invite:[通讯录列表]  ；表示我的通讯录上的人还没有加入脸脸的，可以发短信邀请其加入脸脸
friend: [脸脸用户信息列表]   ；表示我通讯录上的人在脸脸上已经成为好友的，仅用于显示
}
