...
\subsection{脸脸中的聊天消息的三种类型}

\begin{enumerate}
\item chat：个人之间的单聊消息，需要发送送达／已读回执。客户端未收到任何回执，则客户端重发; 服务器端未收到送达／已读回执，则服务器会重发消息。
\item groupchat：聊天室消息，没有回执。客户端未收到自己发出的消息，则重发。
\item normal：服务器推送的通知消息，没有回执，不重发。
\end{enumerate}
...
\begin{verbatim}
该类消息的类型是"normal"，消息id以"VISIT"开头。
该消息的id格式为“VISIT#{uid1},#{uid2}”，其中uid1是访问我的人的id，uid2是我的id。
\end{verbatim}

\subsection{有新人进入聊天室的消息}
\begin{verbatim}
该类消息的类型是"normal"，消息id以"CKN"开头。
该消息的id格式为“CKN#{uid1},#{sid}”，其中uid1是新加入聊天室的人，sid是聊天室的id。
\end{verbatim}
