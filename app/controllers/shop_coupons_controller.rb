...
class ShopCouponsController < ApplicationController
  ...
  def create
    ...
    if @coupon.save
      if @coupon.t.to_i == 1
        ...
        Rails.cache.delete("views/SI#{@coupon.sid}.json")
  ...
  def newly_down
    ...
    hash.merge!(:dat => {"$gt" => params[:date]}) unless params[:date].blank?
  ...
  def newly_use
    ...
    hash.merge!(:cid => params[:cid]) unless params[:cid].blank?
    hash.merge!(:uat => {"$gt" => params[:date]}) unless params[:date].blank?
