...
#  filter_parameter_logging :password
  ...
  def qdh_shop_authorize
    unless session[:shop_id] == 712
      flash[:notice] = "请登录"
      memo_original_url()
      redirect_to( :controller => "shop_login" , :action => "qdh_login")
    else
      redirect_to "/shop_space_index/qdh_noright/0" unless qdh_shop_right_check
    end
  end

  def qdh_shop_right_check
    controller = self.controller_name
    action = self.action_name
    # 商家管理员帐号 千岛湖啤酒
    return true if session[:shop_user_id].nil?
    # "/beeruser_qdhshops" 查看会员列表
    # "/beerorder_qdhshops">查看订单列表</a></td>
    # /beerbucket_qdhshops  桶回收
    # "/beeruser_recharge_qdhshops">查看充值列表</a></td>
    # "/beeruser_gifts">查看赠酒列表</a></td>
    # "/good_qdhshops">物品管理</a></td>
    # "/good_in_qdhshops">入库管理</a></td>
    # "/good_out_qdhshops">出库管理</a></td>
    # "/serveuser_qdhshops">查看客户经理业绩</a></td>
    # "/beerreport_qdhshops">查看会员日报表</a></td>
    # "/sms_qdhshops">查看会员消费信息列表</a></td>
    # "/staff_qdhshops">员工管理</a></td>
    # /qdh_sms_outs     发出的短信列表
    # /sms_in_qdhshops  收到的短信列表
    # 员工类型
    # '销管'  servetype == 1         财务，可操作会员、充值、赠酒及员工全部功能，查看订单列表、报表和下载报表，查看物品列表、报表和下载报表，查看客户经理列表、业绩和下载报表，查看短信
    # '客户经理'  servetype == 2     只可查看会员列表，订单列表，自身客户经理信息、业绩和下载报表，查看短信
    # '客服人员'  servetype == 3     只可查看会员列表，操作订单、物品出入库，查看短信
    # '客户专员'  servetype == 4     司机，无任何权限
    # '管理层'    servetype == 6     只可查看所有列表、报表、下载报表和短信
    # '一渡财务'    servetype == 7     只可查看所有列表、报表、下载报表和短信
    controllers1 = ['beeruser_qdhshops', 'beerorder_qdhshops', 'beeruser_recharge_qdhshops', 'beeruser_gifts', 'serveuser_qdhshops', 'beerreport_qdhshops', 'good_qdhshops', 'good_in_qdhshops', 'good_out_qdhshops', 'qdh_sms_outs', 'sms_qdhshops', 'qdh_sms_outs', 'sms_in_qdhshops', 'staff_qdhshops']
    controllers2 = ['beeruser_qdhshops', 'beerorder_qdhshops', 'serveuser_qdhshops', 'qdh_sms_outs', 'sms_in_qdhshops', 'staff_qdhshops']
    controllers3 = ['beeruser_qdhshops', 'beerorder_qdhshops', 'beerbucket_qdhshops', 'good_qdhshops', 'good_in_qdhshops', 'good_out_qdhshops', 'qdh_sms_outs', 'beerreport_qdhshops',  'sms_qdhshops', 'staff_qdhshops']
    controllers4 = []
    controllers6 = ['beeruser_qdhshops', 'beerorder_qdhshops', 'beeruser_recharge_qdhshops', 'beeruser_gifts', 'serveuser_qdhshops', 'beerreport_qdhshops', 'good_qdhshops', 'good_in_qdhshops', 'good_out_qdhshops', 'sms_qdhshops', 'qdh_sms_outs', 'sms_in_qdhshops', 'staff_qdhshops']
    controllers7 = ['beeruser_qdhshops', 'beerorder_qdhshops', 'beeruser_recharge_qdhshops', 'beeruser_gifts', 'serveuser_qdhshops', 'beerreport_qdhshops', 'good_qdhshops', 'good_in_qdhshops', 'good_out_qdhshops', 'sms_qdhshops', 'qdh_sms_outs', 'sms_in_qdhshops', 'staff_qdhshops']
    shop_user = User.find_by_id(session[:shop_user_id])
    if shop_user.servetype == 1
      if controllers1.include?(controller)
        if controllers1[1] == controller || controllers1[6] == controller || controllers1[7] == controller || controllers1[8] == controller
          return true if (action == "index" || action == "show" || action == "report" || action == "out_xls")
        elsif controllers1[-1] == controller
          return true if (action == "index" || session[:shop_user_id].to_s == params[:id])
        else
          return true
        end
      end
    elsif shop_user.servetype == 2
      if controllers2.include?(controller)
        if controllers2[0] == controller || controllers2[1] == controller
          return true if (action == "index" || action == "show")
        elsif controllers2[-1] == controller
          return true if (action == "index" || session[:shop_user_id].to_s == params[:id])
        else
          return true
        end
      end
    elsif shop_user.servetype == 3
      if controllers3.include?(controller)
        if controllers3[0] == controller
          return true if (action == "index" || action == "show" || action == "find_user")
        elsif controllers3[-1] == controller
          return true if (action == "index" || session[:shop_user_id].to_s == params[:id])
        else
          return true
        end
      end
    elsif shop_user.servetype == 4
      return false
    elsif shop_user.servetype == 6
      if controllers6.include?(controller)
        if controllers6[-1] == controller
          return true if (action == "index" || action == "square" || action == "square_list" || action == "square_out_xls" || session[:shop_user_id].to_s == params[:id])
        else
          return true if (action == "index" || action == "square" || action == "square_list" || action == "square_out_xls" || action == "show" || action == "report" || action == "out_xls")
        end
      end
    elsif shop_user.servetype == 7
      if controllers6.include?(controller)
        if controllers6[-1] == controller
          return true if (action == "index" || action == "square" || action == "square_list" || action == "square_out_xls" || session[:shop_user_id].to_s == params[:id])
        else
