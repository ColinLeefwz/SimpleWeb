...
Mshop.all.each {|x| coll.insert({:id => x.id, :name => x.name, :loc => [x.lat.to_f,x.lng.to_f],:phone => x.phone, :city => x.mcity_id, :cc => x.comment_count, :man => x.manual }) unless x.lng==0}.count
