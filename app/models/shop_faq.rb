...
class ShopFaq
  ...
  def zwyd_delay_send(uid,sid)
    ...
    Resque.enqueue_in(5.seconds,XmppRoomMsg, $gfuid,sid,uid,'点击此处拍照制作你的专属心愿贺卡，还能抽千元红包哦😍',nil, attrs, ext) 
  end
  ...
  def copy_to(option)
    faq_hash = self.clone.attributes.keep_if{|k,v| !k.in?(['_id'])}.merge(option)  
