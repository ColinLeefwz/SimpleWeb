...
class SinaPoiPhoto
  ...
  def self.start
    ...
    num.times do |t|
      SinaPoi.where({:iso_num => {'$gt' => 0}, :photo_fetched => {'$exists' => false} }).sort({iso_num:-1}).skip(t*15).limit(15).to_a.each do |poi|
  ...
  def self.poi_photo_page(token, poiid, err_num = 0)
    ...
    begin
      ...
      $LOG.error "#{Time.now.strftime("%Y-%m-%d %H:%M:%S")} SinaPoiPhoto.poi_photo_page get #{url}错误#{err_num}次. #{$!}"
