...
class NewUser
  ...
  def self.perform(uid,sid,od)
    ...
    ["502e6303421aa918ba000001","50446058421aa92042000002","50ffd0e5c90d8bf7480000b7","51b16477c90d8ba01f0000bb","51b5659dc90d8b511d00002f","51f9e3b9c90d8ba99d000002","51a4b135c90d8be50b000059","52551258c90d8bb6e0000003","524104c0c90d8b05a600000a","5228399fc90d8b4084000002"].each do |to|
  ...
  def self.notify(uid,sid, to, od, ud, gender=0, city=nil)
    ...
    Xmpp.send_chat(uid,to,"#{user.show_gender}:#{od}:#{from}#{os} #{shop.name} #{shop.city_fullname}", "FEED#{uid}", " NOLOG='1' NOPUSH='1' SID='#{shop.id}' SNAME='#{shop.name}' ", "<x xmlns='dface.shop' SID='#{shop.id}' SNAME='#{shop.name}' ></x>")
  end
