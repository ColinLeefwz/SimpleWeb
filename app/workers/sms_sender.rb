...
class SmsSender
  ...
  def self.send_sms_ihuiyi(phone, text, rtry=1)
      ...
      if rtry>0
        return send_sms_ihuiyi(phone, text,0)
      else
  ...
  def self.send_sms_xuanwu(phone, text, rtry=1)
      return true if ENV["RAILS_ENV"] != "production"
      ...
      if rtry>0
        return send_sms_xuanwu(phone, text,0)
      else
  ...
  def self.send_sms_mandao(phone, text, rtry=1)
      return true if ENV["RAILS_ENV"] != "production"
      ...
      if rtry>0
        return send_sms_mandao(phone, text,0)
      else
