...
class CkeditorController < ActionController::Base
  ...
  UPLOAD_FOLDER = "/files/" + Time.zone.now.strftime("%Y%m")
  ...
  def upload_file
    begin
      ...
      # Resource.create(:filename => (Time.zone.now.to_i.to_s + "." + @new_file.original_filename.split(".")[-1]), :mime => @ftype, :created_at => Time.now)
    ...
    <html>
      <body>
        <script type="text/javascript">
          window.parent.CKEDITOR.tools.callFunction(1, "#{url_for(:controller => "/").gsub(%r{/$},'')}#{uploaded_file_path}");
        </script>
      </body>
    </html>'
  end
