...
function ImgList(n, isajax){
	...
	}else if(funs>8&&funs<(num-8)){
	    piclist.style.left=-(piclist.getElementsByTagName("li")[0].offsetWidth)*Math.floor(funs/8)*8+"px";
		document.getElementById("LA").className="la";
		document.getElementById("RA").className="ra";
	}else if((num-funs)<9){/**用于判断否是是图片序列末端的图片**/
		piclist.style.left=-parseInt(piclist.getElementsByTagName("li")[0].offsetWidth)*Math.floor(funs/8)*8+"px";
...
function ArrowUp(){
	...
	if(funs2>=8&&(funs2%8==0)){
		if(funs2>Math.floor(num/8)*8)return;/**指针位于图片列表末尾几张图片时取消循环**/
		clearInterval(run);run=setInterval("Run(false)",60);
	}else if(funs2==8){clearInterval(run);run=setInterval("Run(false)",60);}
...
function ArrowDown(){
	...
	piclist.getElementsByTagName("li")[funs2].className="hover";
	OpenPic(funs2, true);
	if(funs2>=8&&(funs2%8==0)){clearInterval(run);run=setInterval("Run(true)",60);}
