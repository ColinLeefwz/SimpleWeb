...
function ImgList(n){
	...
	}else if(funs>6&&funs<(num-6)){
	    piclist.style.left=-(piclist.getElementsByTagName("li")[0].offsetWidth)*Math.floor(funs/6)*6+"px";
		document.getElementById("LA").className="la";
		document.getElementById("RA").className="ra";
	}else if((num-funs)<7){/**用于判断否是是图片序列末端的图片**/
		piclist.style.left=-parseInt(piclist.getElementsByTagName("li")[0].offsetWidth)*Math.floor(funs/6)*6+"px";
...
function ArrowUp(){
	if(funs2<=0){funs=funs2=num-1;ImgList(funs);document.getElementById("LA").className="la";return;}
	...
	if(funs2>=6&&(funs2%6==0)){
		if(funs2>Math.floor(num/6)*6)return;/**指针位于图片列表末尾几张图片时取消循环**/
		clearInterval(run);run=setInterval("Run(false)",60);
	}else if(funs2==6){clearInterval(run);run=setInterval("Run(false)",60);}
	if(funs2==0){document.getElementById("LA").className="la nla";}
	
}
function ArrowDown(){
	if(funs2>=(num-1)){funs=funs2=0;ImgList(funs);return;}
	piclist.getElementsByTagName("li")[funs2].className="";
	funs2++;
	piclist.getElementsByTagName("li")[funs2].className="hover";
	OpenPic(funs2);
	if(funs2>=6&&(funs2%6==0)){clearInterval(run);run=setInterval("Run(true)",60);}
