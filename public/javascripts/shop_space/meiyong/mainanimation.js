...
var numz=document.getElementById("stage").getElementsByTagName("div").length;
var autoPlay=false;
var bgimages=[];
init();
play();
...
function stage(){this.frames=[numz];
...
function frame(b,a,f){
	...
	this.id="f"+b;this.elem=document.getElementById(this.id);
	this.activeOverlay=-1;
	this.imgLoaded=false;this.overlays=[f];
	var d=1;
	for(d;d<=f;d++){
...
function init(){
	...
	c.s.frames[0].imgLoaded=true;c.selectedIndex=1;
...
function hideOtherVisibleOverlays(b){
	...
	for(var a=0;a<d.length;a++){
		...
		if(d[a].style.display=="block"&&d[a].id!=b){ d[a].style.display="none"; }
...
function getElementsByClassName(b,g,a){
	...
	for(var d=0;d<f.length;d++){
		...
		if(h.test(e.className)){ j.push(e); }
...
function slide(b,a){
	...
	if(slidetimer!=undefined||slidetimer!=null){ clearTimeout(slidetimer); }
...
function reposition(b,a){
	...
	c.s.frames[b-1].elem.style.left=660*a+"px";
...
function highlightA(a){
	...
	if(a.className=="arrow2"){ a.className="arrow2"; }
	
	if(a.className=="arrow1"){ a.className="arrow1"; }

}

function dehighlightA(a){
	
	if(a.className=="arrow2"){
		
		a.className="arrow2";
		
	}else{
		
		if(a.className=="arrow1"){
...
function prevF(){
	...
	if(c.busy||a==c.selectedIndex){ return; }
