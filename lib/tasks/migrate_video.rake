...
task :migrate_videos => :environment do
  ...
  video_types.each do |type|
    ...
    (type.constantize).all.each do |t|
      ...
      if (t.attached_video_sd_file_name.present? or t.attached_video_hd_file_name.present?)  # if it has an video
        ...
        puts "****** current_cover_path #{t.class.name}  #{current_cover_path} ******"
        ...
        puts "****** expect_cover_path #{t.class.name}  #{expect_cover_path} ******"
